...
def flux(request):
    ...
    ticket_list = (
        Ticket.objects.filter(user__in=followed_usernames)
        .annotate(
            review_user=F("review__user__username"),
...
def create_review(request):
    ...
    if request.method == "POST":
        ...
        if review_form.is_valid():
            linked_ticket = Ticket.objects.filter(user__exact=request.user).order_by(
                "-time_created"
            )[0]
            ...
            new_review.save()
            messages.success(
                request, f"Critique validée pour l'oeuvre suivante: {new_ticket.title}"
            )
            return redirect("../")
...
def create_ticket(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_ticket.save()
            messages.success(
                request, f"Critique demandée pour l'oeuvre suivante: {new_ticket.title}"
            )
            return redirect("../")
    context = {"form": form}
